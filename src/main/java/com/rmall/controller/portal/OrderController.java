...
public class OrderController {
    ...
    public ServerResponse pay(HttpServletRequest httpServletRequest, long orderNo, HttpServletRequest request){
        //传入HttpServletRequest的作用，为了获取servlet上下文，获取upload那个文件夹，然后将自动生成的
        //二维码传到FTP服务器上，返回二维码的图片地址，前端将其进行展示用于扫码支付
        String loginToken = CookieUtil.readLoginToken(httpServletRequest);
        if(StringUtils.isEmpty(loginToken)){
            return ServerResponse.createByError("用户未登录，获取用户信息失败");
        }
     ...
     * 支付宝回调接口
     * */
    @RequestMapping(value = "alipay_callback.do",produces = "application/json; charset=utf-8")
    @ResponseBody
    public Object alipayCallback(HttpServletRequest request){
        ...
        /**
         * 验证回调的通知是否是支付宝发的，并且还要避免重复通知
         * */
        params.remove("sign_type");
        try {
            boolean alipayRSACheckedV2 = AlipaySignature.rsaCheckV2(params, Configs.getAlipayPublicKey(), "utf-8",Configs.getSignType());
            if(!alipayRSACheckedV2){
                return ServerResponse.createByError("非法请求，验证不通过，这是恶意请求");
